...
const loadProducts = () => {
  const srcResult = document.getElementById('input-field').value;
  ...
  const srcValue = data.filter(single => single.title.toLowerCase().includes(srcResult.toLowerCase()));
  console.log();

  if(srcResult === '' || +srcValue === 0){
  ...
  showProducts(srcValue);
    // console.log(data);
};

//search anf get value
